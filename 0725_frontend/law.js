...
function stock_page(stock_num) {
   
    box_0F_data({ ID: "ZongPingBaoBiao", ZongPingBaoBiao_stock_num: stock_num })
}
$(document).ready(function () {
    let title_tmpl = `
        <a href="#" class="list-group-item list-group-item-action p-0 disabled">查詢個股</a>
    `
    let item_tmpl = `
        <a href="#" class="stock_search_item list-group-item list-group-item-action p-2">5425 台辦</a>
    `
    // $(".search-form").click(function (e) { 
    //     e.preventDefault();
    //     $.ajax({
    //         type: "POST",
    //         url: "http://43.254.19.106:5107/stock_search",
    //         data: {
    //             search_mode:"num",
    //             search_keyword:"5"
    //         },
    ...
    //         }
    //     });
    //     $(".stock_search_output").append("fff");
    // });
    $(".search-form input[name=query]").keyup(function (e) { 
        ...
        $.ajax({
            ...
            dataType: "json",
            success: function (response) {
                $(".stock_search_output").html("");
                console.log(response)
                let title_tmpl_obj = $.parseHTML( title_tmpl )
                $(".stock_search_output").append(title_tmpl_obj);
                for(let i of response.values){
                    let item_tmpl_obj = $.parseHTML( item_tmpl )
                    $(item_tmpl_obj).html( `${i[0]} ${i[1]}`);
                    // $(item_tmpl_obj).attr("onclick",`stock_page("${i[0]}")`);
                    $(".stock_search_output").append(item_tmpl_obj);

                }
                $(".stock_search_item").click(function (e) { 
                    e.preventDefault();
                    $(".search-form input[name=query]").val($(this).html());
    ...
    $(".search-form input[name=query]").focus(function (e) { 
        $(".search_output").show();
    });
    $(".search-form button[type=submit]").click(function (e) { 
        e.preventDefault();
        $(".search-form input[name=query]").focusout();
        let query_stock_str = $(".search-form input[name=query]").val().trim();
        let stock_num = query_stock_str.split(" ")[0];
        let stock_name = query_stock_str.split(" ")[1];
        stock_page(stock_num);
