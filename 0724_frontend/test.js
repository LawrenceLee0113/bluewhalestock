...
MeiKuoTsungChing       = ["消費指標","M1cb","製造投資","M1bi","建築指標","M1bc","物價指數","M1gp",];
MaoYiShuJu             = [];
ShangPinHangQing       = [];
...
//小產業分類
Semiconductor          = ['矽智財','設計服務','記憶體','記憶體控制IC','電源管理IC','驅動IC','網路通訊IC ','影音IC','影像感測IC','觸控IC','微控制器MCU','輸出入介面IC','二極體','功率電晶體','記憶體模組','晶圓','磊晶','網通晶圓','晶圓代工 ','封測材料','導線架','IC封測',];
Optoelectronics        = ['背光模組 ','面板材料','面板','觸控面板模組','顯示器','LED晶粒 ','LED封裝','LED燈具 ','光學鏡頭','光碟片','攝影機 ',];
...
ElectronicDistributors = ['3C通路','半導體通路','設備通路','電子材料通路','電子零件通路',];
...
Chemical               = ['天然氣供應','加油站','化工原料','塑膠原料','石化原料','合成樹脂','染料塗料','化學品','塑膠加工','農業化學',];
...
function box_with_1F(event){
  ...
  switch (id) {
    ...
    case "MeiKuoTsungChing":{nameIDList = MeiKuoTsungChing ;break;};
  ...
  dataBox_display="";
  // 按下按鈕回傳左導覽列第一個項目的ID
  box_1F_data({ID:nameIDList[1],father_ID:id});
  ...
  getleftnavClass.forEach(iCl => {
    iCl.addEventListener('click', function leftnavClassClick(event) {
      ...
      box_1F_data({ID:this.id,father_ID:id})
    });
  });
...
function F_for_box_with_2F(){
  ...
  for (var i = 0; i < nameIDList2.length; i++) {
    ...
    // 第一個按鈕顏色不一樣 class 加上 activeButton 改變文字顏色
    if(String(middleClassID+String(i+1)) == smallClassID){
      dataBox_display += 'activeButton '
    }
...
function box_with_2F(event){
  ...
  for (var i = 0; i < (ChanYeTzShiun.length)/4; i++) {
    ...
    // 第一個按鈕顏色不一樣 class 加上 activeButton 改變文字顏色
    if(ChanYeTzShiun[i*2+1] == middleClassID){
      dataBox_display += 'activeButton'
    }
...
// 依照類型觸發3種不同的 Ｆ（）
// ===========================類型0===========================
function box_0F_data({ID,ZongPingBaoBiao_stock_num}){
  switch (ID) {
    // 首頁
    ...
    case "ZongPingBaoBiao":{
        console.log(ID,"loading..")
        switch (loaded_leftNavScripts.includes(ID)){
        case false:
            $.getScript("/assets/js/page/ZongPingBaoBiao_js.js", function (script, textStatus, jqXHR) {
            changeZongPingBaoBiao({ID,stock_num:ZongPingBaoBiao_stock_num})
            })
            break;
        case true:
            changeZongPingBaoBiao({ID,stock_num:ZongPingBaoBiao_stock_num})
...
var loaded_leftNavScripts = []
var nowLeftNavName = ""

function box_1F_data({ID,father_ID}){
    switch (father_ID) {
      // =========大盤指標========
      case "DaPanZhiBiao":{console.log(father_ID,"loading..");break;}
      // =========權值個股========
      case "QuanZhiGeGu":{
        console.log(father_ID,"loading..")
          switch (loaded_leftNavScripts.includes(father_ID)){
            case false:
              $.getScript("/assets/js/page/QuanZhiGeGu_js.js", function (script, textStatus, jqXHR) {
                changeQuanZhiGeGu(ID)
              })
              break;
            case true:
              changeQuanZhiGeGu(ID)
      ...
      case "ShiChangZiXun":{
          ...
          switch (loaded_leftNavScripts.includes(father_ID)){
            case false:
              $.getScript("/assets/js/page/ShiChangZiXun_js.js", function (script, textStatus, jqXHR) {
                changeShiChangZiXunPage(ID)
              })
              break;
            case true:
              changeShiChangZiXunPage(ID)
      ...
      // =========美國總經========
      case "MeiKuoTsungChing":{console.log(father_ID,"loading..");break;}
      // =========個股排行========
      case "GeGuPaiHang":{console.log(father_ID,"loading..");break;}
    }
  // switch (ID) {
  //   // =========大盤指標========
  //   // 大盤營收
  ...
  //   // =========權值個股========
  //   // 市值Top10
  //   case "Q1t1":{QuanZhiGeGu_load(ID);break;}
  //   // 市值Top20
  //   case "Q1t2":{QuanZhiGeGu_load(ID);break;}
  //   // 市值Top30
  //   case "Q1t3":{QuanZhiGeGu_load(ID);break;}
  //   // 市值Top40
  //   case "Q1t4":{QuanZhiGeGu_load(ID);break;}
  //   // 市值Top50
  //   case "Q1t5":{QuanZhiGeGu_load(ID);break;}
  //   // =========市場資訊========
  //   // 新股上市上櫃
  //   case "S1ns":{page_change_1F(ID);break;}
  //   // 董監持股轉讓
  //   case "S1is":{page_change_1F(ID);break;}
  //   // 異常推介個股
  //   case "S1ss":{page_change_1F(ID);break;}
  //   // 注意處置股票
  //   case "S1as":{page_change_1F(ID);break;}
  //   // 分盤交易股票
  //   case "S1sb":{page_change_1F(ID);break;}
  //   // 除權除息預告
  //   case "S1dp":{page_change_1F(ID);break;}
  //   // 今日重大訊息
  //   case "S1tb":{page_change_1F(ID);break;}
  //   // 公司自結盈餘
  //   case "S1ce":{page_change_1F(ID);break;}
  //   // =========經濟指標========
  //   // 消費指標
  //   case "M1cb":{alert(ID);break;}
  //   // 製造投資
  //   case "M1bi":{alert(ID);break;}
  //   // 建築指標
  //   case "M1bc":{alert(ID);break;}
  //   // 物價指數
  //   case "M1gp":{alert(ID);break;}
  //   // =========個股排行========
  //   // 營收成長
  ...
  switch (father_ID) {
    ...
    case "YingShouZongLan":{
      console.log(father_ID,"loading..")
          switch (loaded_leftNavScripts.includes(father_ID)){
            case false:
              $.getScript("/assets/js/page/YingShouZongLan_js.js", function (script, textStatus, jqXHR) {
                changeYingShouZongLan({ father_ID, ID })
              })
              break;
            case true:
              changeYingShouZongLan({ father_ID, ID })
    ...
    case "JhangDieZongLan":{
      console.log(father_ID,"loading..")
          switch (loaded_leftNavScripts.includes(father_ID)){
            case false:
              $.getScript("/assets/js/page/JhangDieZongLan_js.js", function (script, textStatus, jqXHR) {
                changeJhangDieZongLan({ father_ID, ID })
              })
              break;
            case true:
              changeJhangDieZongLan({ father_ID, ID })
    ...
    // 個股股價
    case "GeGuGuJia"      :{page_change_2F({father_ID,ID});break;};
    // 彙總報表
